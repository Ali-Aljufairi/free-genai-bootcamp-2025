...
def display_product_comparisons(
    comparisons: List[Dict], best_product: Dict, youtube_link: str = None
):
    """
    ...
    with tab2:
        ...
        for product in comparisons:
            ...
            fig.add_trace(
                go.Scatterpolar(
                    r=values, theta=categories_closed, fill="toself", name=product_name
                )
            )
    ...
    with tab3:
        ...
        for product in comparisons:
            product_name = product.get("product_name", "Unknown")
            reviews_summary = product.get(
                "reviews_summary", "No review information available."
            )
...
def display_streamlit_app(data: Dict[str, Any]):
    ...
    try:
        ...
        st.set_page_config(
            page_title="ShopGenie Product Recommendation", page_icon="🧞", layout="wide"
        )
