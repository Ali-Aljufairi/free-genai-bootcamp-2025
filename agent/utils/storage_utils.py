...
def store_output(data: Dict[str, Any], email: str, output_type: str = "email") -> str:
    ...
    storage_dir = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "data", "outputs"
    )
    ...
    data_with_timestamp = {"timestamp": int(time.time()), "data": data}

    # Write the data to the file
    with open(file_path, "w") as f:
        json.dump(data_with_timestamp, f, indent=2)

    log(f"Stored {output_type} output to {file_path}")
    return file_path
